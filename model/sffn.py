...
class ChannelFocal(nn.Module):
    def __init__(self, in_dim, init_dim=8, focal_level=4, focal_factor=2, bias=True):
        ...
        self.head_conv = nn.Sequential(nn.Conv2d(in_dim, sum_dim, 1, 1, 0, bias=bias))
...
class SpaBlock(nn.Module):
    def __init__(self, in_size, out_size, levelnum=4):
        ...
        self.layer_3 = nn.Sequential(*[
            ...
            nn.Conv2d(out_size, out_size, 3, 1, 1, groups=out_size),
...
class MFRFC(nn.Module):
    def __init__(self, in_channels, order=0.5):
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels),
...
class UncertaintyEstimation(nn.Module):
    def __init__(self, channels, T, q, cnum=3):
        ...
        self.T = T
        self.q = q
        self.conv = nn.Sequential(nn.Conv2d(channels, channels * 2, 3, 1, 1, groups=channels, bias=True),
                                  nn.Conv2d(channels * 2, channels * 2, 1, 1, 0, bias=True))
...
class MergeBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.merge = nn.Sequential(nn.Conv2d(2*in_ch, out_ch, 1, 1), nn.GELU(),
                                   nn.Conv2d(out_ch, out_ch, 3, 1, 1, groups=out_ch),
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        spa_eu, spa_mean = self.spa_ue(x_spa)
        ...
        x_fre = x_fres + x_fre2
        fre_eu, fre_mean = self.fre_ue(x_fre)
